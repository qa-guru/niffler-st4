...
public class UsersQueueExtension implements BeforeEachCallback, AfterTestExecutionCallback, ParameterResolver {
    ...
    private static List<Parameter> getParameters(ExtensionContext context) {
        ...
        methods.add(context.getRequiredTestMethod()); // сигнатура тестового метода
        Arrays.stream(context.getRequiredTestClass().getDeclaredMethods()) // метод помеченый аннотацией
                .filter(m -> m.isAnnotationPresent(BeforeEach.class))
                .forEach(methods::add);

        // Собрали 2 метода, тестовый и BeforeEach
        // Тащим из этих методов параметры
    ...
    public void beforeEach(ExtensionContext context) throws Exception {
        ...
        for (Parameter parameter : parameters) {
            ...
            if (Objects.nonNull(userType)) {
                ...
                Assertions.fail("В параметрах не найдено значение типа User.class");
            }
        }
